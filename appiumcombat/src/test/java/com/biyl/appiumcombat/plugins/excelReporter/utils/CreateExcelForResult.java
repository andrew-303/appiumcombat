package com.biyl.appiumcombat.plugins.excelReporter.utils;
...
public class CreateExcelForResult {
	...
	public static void createExcelReport(List<XmlSuite> xml,List<ISuite> suites,String dir){
		...
		cellC2.setCellStyle(setContentCellBorder(workbook));
		//合并测试项目名称的单元格
			...
			for (ISuiteResult r : tests.values()) {	//多少个模块第一层循环
				...
				if(failTestNum != 0 ){//处理失败的
					...
					for (ITestNGMethod im : overview.getFailedTests().getAllMethods()) {//模块下有多少个用例循环
						...
						//设置用例运行的状态
						workbook.getSheet(function).getRow(count+2).getCell(2).setCellValue(new XSSFRichTextString("Faild"));
						//设置log,这是一个超链接
				...
				if(skipTestNum!=0){//处理跳过的
					...
					for(ITestNGMethod im:overview.getSkippedTests().getAllMethods()){//模块下有多少个用例循环
						...
						xcs.setFillForegroundColor(XSSFCellStyle.SOLID_FOREGROUND);
	...
	public static CellStyle setCellStyle2(XSSFWorkbook workbook) {
		XSSFCellStyle xssfCellStyle = workbook.createCellStyle();
		XSSFFont font = workbook.createFont();
		font.setFontName("微软雅黑");
		font.setColor((short) 8);
	...
	public static XSSFCellStyle setCellStyle1(XSSFWorkbook workbook) {
		...
		xssfColor.setRgb(rgb);
		xssfCellStyle.setFillForegroundColor(xssfColor);//设置前端颜色
		xssfCellStyle.setFillPattern(XSSFCellStyle.SOLID_FOREGROUND);//设置图案样式
		setCellBorder(workbook,xssfCellStyle);
		return xssfCellStyle;
	}
	...
	public static CellStyle setCellStyle4(XSSFWorkbook workbook2) {
		XSSFCellStyle xssfCellStyle = workbook.createCellStyle();
		//设置cell样式
