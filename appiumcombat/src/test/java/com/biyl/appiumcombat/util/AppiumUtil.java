...
/**
 * 01.appium api封装
 * 这个类的目的是为了使框架中所有的基本操作都从这个类中去调用，实现统一性
 * @author HZFI_DC
 *
 */
...
//				break;
	...
	public void submit(By by){
		...
		try {
			...
			logger.info("在元素：" + getLocatorByElement(element, ">") + "做的提交操作失败" ,e);
			Assert.fail("在元素：" + getLocatorByElement(element, ">") + "做的提交失败",e);			
		}
	...
	public void isContains(String actual,String expected){
		try{
			...
			logger.error("The["+actual+"] is not contains ["+ expected +"]");
			Assert.fail("The["+actual+"] is not contains ["+ expected +"]");
		}
		logger.info("The[" + actual + "] is not contains [" + expected + "]");
	}
	...
	*/
	...
	public void swipe(int beginX, int beginY, int endX, int endY){
		TouchAction touchAction = new TouchAction(driver);
		try {
			touchAction.press( beginX, beginY).moveTo(endX, endY).release().perform();
		} catch (Exception e) {		
			e.printStackTrace();
		}
	}
	...
	public void DragAndDrop(By dragElement,By dropElement){
		...
		taction.press(findElement(dragElement)).perform();
		taction.moveTo(findElement(dropElement)).release().perform();
	}
	...
	public void zoomAndPinch(int beginX,int beginY,int endX,int endY){
		int scrHeight = driver.manage().window().getSize().height;	//屏幕高
		int scrWidth = driver.manage().window().getSize().width;	//屏幕宽
		MultiTouchAction multiTouch = new MultiTouchAction(driver);	//多点触控
	...
	public boolean isAppInstalled(String appPackage){
		if(driver.isAppInstalled(appPackage)){
			logger.info(appPackage+"App已经安装");
			return true;			
		}else{
			logger.info(appPackage+"App未安装");
			return false;
		}
	}
